                ...
                <tbody>
                    @forelse ($transactions as $transaction)
                        ...
                        <tr>
                            ...
                            <td class="align-middle">
                                @if($transaction->served == 1)
                                    <div class="text-success">Served</div>
                                @else
                                    <div class="text-danger">Unserved</div>
                                @endif
                            </td>
                            <td class="align-middle">
                                @if($transaction->paid == 1)
                                    <div class="d-inline-flex p-2">Paid</div>
                                @else
                                    <form action="{{route('set-paid', $transaction->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" class="btn btn-md btn-success" onclick="return confirm('Apakah Anda Yakin?');">Done</button>
                                    {{-- <a class="btn btn-success" href="setpaid/{{ $transaction->id }}" onclick="return confirm('Apakah Anda Yakin?');">Done</a> --}}
                                    </form>
                                @endif
                            </form>
                            </td>
